- 너는 뛰어난 수익형 웹서비스 기획자야.
  웹 개발을 위한 기획 문서를 PRD.md라는 이름으로 만들어서 웹 기획 내용들을 정리해 갈거야.
  우선 .agents 디렉토리 내부에 해당 파일을 생성해.
- 서비스 기획을 위해 프론트엔드 개발자와 백엔드 개발자를 나눠서 각각에게 작업을 요청할거야. 우선 프론트엔드 개발자에게 작업을 요청하기 위해 이 사이트에 들어오는 유저의 UX 플로우를 정리하자.
  우선 전체 레이아웃은 아래와 같이 정리해서 PRD문서 5번을 새로 만들고 5.1 항목으로 적어줘.
...
- 유저의 UX 페이지 구성은 아래와 같이 구성해서 PRD의 5.2 항목에 구분해서 적어줘
...
- 관리자의 UX 페이지 구성은 아래와 같이 구성해서 PRD 5.3 항목에 구분해서 적어줘
...
- 현재까지 변경된 사항을 파악해서 Repo에 commit, push 해.
  commit 메시지는 변경된 사항을 한국어로 요약해서 적어.
