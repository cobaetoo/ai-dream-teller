...
// 더미 비회원 주문 내역 데이터
const MOCK_GUEST_HISTORY = [
  {
    id: "ord_g1",
    date: new Date("2026-03-11"),
    title: "끝없는 황금빛 들판을 달리는 꿈",
    summary: "현재 진행 중인 프로젝트나 목표에서 큰 성과를 거두게 될 것임을 암시하는 길몽입니다.",
  ...
  },
  {
    ...
    title: "심해 속에서 발견한 거대한 도시",
    summary: "내면의 깊은 창의성과 잠재력이 깨어나고 있음을 나타내며, 새로운 도전을 시작하기 좋은 시기입니다.",
    thumbnail: null, // 이미지 없음
    price: 1500,
  },
];
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="px-4 py-2 bg-white rounded-2xl border border-slate-100 shadow-sm inline-flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-slate-700">010-****-1234</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              ...
              총 {MOCK_GUEST_HISTORY.length}건의 주문 내역
            </h3>
          </div>

          {MOCK_GUEST_HISTORY.length > 0 ? (
            <div className="space-y-4">
              {MOCK_GUEST_HISTORY.slice(0, visibleCount).map((item) => (
                <Link
                  key={item.id}
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:border-indigo-200 transition-all duration-300 flex items-center gap-6">
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] font-bold text-slate-300">#{item.id}</span>
                      </div>
          ...
          {visibleCount < MOCK_GUEST_HISTORY.length && (
